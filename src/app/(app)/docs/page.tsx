...
import type { FilterPreset, SortField, ViewMode } from '@/lib/types';
...
function FileListItem({ file }: { file: ReturnType<typeof useFileStore.getState>['recentFiles'][number] }) {
  return (
    <FileCard file={file} />
  );
}
...
export default function DocsPage() {
  ...
  /* Filter & sort */
  const files = useMemo(() => {
    let result = recentFiles.filter((f) => f.type === 'doc');

    if (filter === 'mine') result = result.filter((f) => f.owner_id === 'usr-1');
    else if (filter === 'shared') result = result.filter((f) => f.org_id !== null);
    else if (filter === 'starred') result = result.filter((f) => f.is_starred);
    ...
    >
      ...
      <div className="flex items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--color-text-secondary)]">
            Write, edit, and collaborate on documents
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Create your first document to start writing and collaborating with your team."
